...
# Source .env at the very top unless explicitly disabled for controlled redeploys.
if [[ "${PMOMAX_SKIP_ENV:-0}" != "1" && -f "$ROOT_DIR/.env" ]]; then
  set -a
...
# ------------------------------------------------------------
if [[ ! -f "$ROOT_DIR/Dockerfile.cloudrun" ]]; then
  echo "ERROR: Dockerfile.cloudrun is required for the PMOMax runtime image build."
  exit 1
fi
...
rsync -a --delete \
  --filter="include /Dockerfile.cloudrun" \
...
# Copy ignore files
cp -f "$ROOT_DIR/Dockerfile.cloudrun" "$STAGE_DIR/Dockerfile"
