import { sanitizeUntrustedText, redactSecrets } from './security/promptDefense.js';
...
const sanitizeNotesForExport = (notes) => {
...
import { Document, Packer, Paragraph, HeadingLevel, TextRun, Table, TableRow, TableCell, WidthType, AlignmentType, ShadingType, TableLayoutType, } from 'docx';
import { SECTION_ORDER, SECTION_TITLES, isPopulated, toGanttDataURL, TABLE_HEADERS, } from './exportUtils.js';
...
const formatInline = (v) => {
    ...
    if (Array.isArray(v))
        return sanitizePdfText(v.map(formatInline).filter(Boolean).join(', '));
    if (typeof v === 'object') {
        const name = v.name || v.fullName || v.displayName;
        const role = v.role || v.title;
        const email = v.email || v.contact;
        if (name && role && email)
            return sanitizePdfText(`${name} — ${role} (${email})`);
        if (name && role)
            return sanitizePdfText(`${name} — ${role}`);
        if (name && email)
            return sanitizePdfText(`${name} (${email})`);
        if (name)
            return sanitizePdfText(String(name));
        try {
...
export async function exportToPdf(pid, generalNotes) {
    var _a, _b, _c, _d;
    // SSR / environment guard
    if (typeof window === 'undefined' ||
        !window.jspdf ||
        !window.jspdf.jsPDF) {
        throw new Error('PDF library (jsPDF) is not loaded or not available in this environment.');
    }
    ...
    const title = sanitizePdfText(((_a = pid.titleBlock) === null || _a === void 0 ? void 0 : _a.projectTitle) || 'Project Initiation Document');
    const subtitle = sanitizePdfText(((_b = pid.titleBlock) === null || _b === void 0 ? void 0 : _b.subtitle) || 'Project Initiation Document');
    const generatedOn = sanitizePdfText(((_c = pid.titleBlock) === null || _c === void 0 ? void 0 : _c.generatedOn) || '');
    ...
    const addSectionHeader = (text) => {
        ...
        if (currentY > MARGIN + 10)
            currentY += 10; // compact spacing between sections
    ...
    const embedGanttIfAvailable = async () => {
        try {
            ...
            doc.addImage(ganttDataUrl, 'PNG', MARGIN, currentY, CONTENT_WIDTH, imgHeight, undefined, 'FAST');
    ...
    for (const key of (Array.isArray(SECTION_ORDER) ? SECTION_ORDER : [])) {
        ...
        if (!isPopulated(value))
            continue;
        const titleText = (_d = SECTION_TITLES[key]) !== null && _d !== void 0 ? _d : key;
        ...
        // Array-of-object -> table (if header mapping available)
            ...
            const body = value.map((item) => headerKeys.map((k) => formatInline(item === null || item === void 0 ? void 0 : item[k])));
            addTable(head, body);
        }
...
const createDocxTable = (head, rows, colWidths) => {
    const headerRow = new TableRow({
        children: head.map((h, idx) => {
            const w = colWidths === null || colWidths === void 0 ? void 0 : colWidths[idx];
...
export async function exportToWord(pid, generalNotes) {
    var _a, _b, _c, _d;
    const title = ((_a = pid.titleBlock) === null || _a === void 0 ? void 0 : _a.projectTitle) || 'Project Initiation Document';
    const subtitle = ((_b = pid.titleBlock) === null || _b === void 0 ? void 0 : _b.subtitle) || 'Project Initiation Document';
    const generatedOn = ((_c = pid.titleBlock) === null || _c === void 0 ? void 0 : _c.generatedOn) || '';
    const children = [];
    // Title page
    ...
    for (const key of (Array.isArray(SECTION_ORDER) ? SECTION_ORDER : [])) {
        ...
        if (typeof value === 'string') {
            ...
            const headerKeys = Object.keys(headerMap);
            const head = headerKeys.map((k) => headerMap[k]);
            const rows = value.map((item) => headerKeys.map((k) => formatInline(item === null || item === void 0 ? void 0 : item[k])));
 ...
 */
export async function exportToJson(pid, generalNotes) {
    var _a;
    try {
        // SSR / environment guard
        if (typeof window === 'undefined' || typeof document === 'undefined') {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `${((_a = pid.titleBlock) === null || _a === void 0 ? void 0 : _a.projectTitle) || 'PMOMax_PID'}.json`;
        document.body.appendChild(a);
        a.click();
        a.remove();
        // Clean up
        ...
        if (typeof window !== 'undefined') {
            alert('JSON export failed. Please try again or check the console for details.');
        }
        else {
            throw err;
        }
