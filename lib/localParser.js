...
const _norm = (s) => (s || "").replace(/[^a-z0-9]/gi, "").toLowerCase();
...
const parseTable = (text) => {
    ...
    if (headerLine.includes('\t'))
        ...
        delimiter = '|'; // Pipe without markdown structure
    else if (headerLine.match(/ {2,}/))
...
const parseBulletList = (text) => {
    ...
    const rows = lines
        ...
        .filter(Boolean)
        .map(match => ({ item: match[1].trim() }));
    return rows.length > 0 ? rows : null;
};
export const localParse = (text) => {
    try {
        const fields = Object.fromEntries(FIELD_KEYS.map(k => [k, ""]));
        const tables = Object.fromEntries(Object.keys(TABLE_KEYS).map(k => [k, []]));
        const sections = [];
        ...
        if (headingsFound === 0) {
            ...
            if (!fields['fld-project-name']) {
                ...
                if (firstLine)
                    fields['fld-project-name'] = firstLine.trim();
            }
        ...
        for (const section of sections) {
            ...
            if (section.key.startsWith('tbl-') && tableData) {
                ...
                const mappedTable = tableData.map((row, index) => {
                    const newRow = { id: tables[section.key].length + index + 1 };
