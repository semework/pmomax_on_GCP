...
export function isPopulated(value) {
    ...
    if (Array.isArray(value))
        return value.length > 0 && value.some((v) => isPopulated(v));
    if (typeof value === 'object')
        return Object.values(value).some((v) => isPopulated(v));
    return true;
}
...
export const TABLE_HEADERS = {
    ...
    budgetCostBreakdown: { item: 'Item', category: 'Category', cost: 'Cost' },
...
export async function toGanttDataURL(format = 'png', scale = 2) {
    var _a;
    /**
    ...
    const svgDirect = root instanceof SVGSVGElement ? root : root.querySelector('svg');
    // Try Plotly export when possible (best fidelity)
    const plotDiv = (((_a = root.classList) === null || _a === void 0 ? void 0 : _a.contains('js-plotly-plot'))
        ? root
        : root.querySelector('.js-plotly-plot'));
    const Plotly = window === null || window === void 0 ? void 0 : window.Plotly;
...
export async function downloadImage(format) {
