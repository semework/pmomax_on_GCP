...
{/* Cards row */}
  ...
  const formatJustification = (row: any) => {
    ...
    return j || 'Estimated from comparable delivery work; verify with local rates, vendor quotes, or internal benchmarks.';
  };
    ...
    <main className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden p-4 md:p-6 space-y-6 text-white">
      ...
      {visibleScopeSection && (
        <Section id="scope" title="04 — Scope & Deliverables" onHelp={onHelp} helpContext="scope">
          ...
          {Array.isArray(deliverablesOutputs) && deliverablesOutputs.length > 0 && (
            <Field title="Deliverables / Outputs">
              <ul className="list-disc list-inside text-sm text-white space-y-1">
                {deliverablesOutputs.map((item: any, idx: number) => (
                  <li key={idx}>
                    {typeof item === 'string' ? item : item?.name || item?.deliverable || String(item || '')}
                    {item?.owner ? <span className="text-xs text-amber-300"> — {item.owner}</span> : null}
                    {item?.dueDate ? <span className="text-xs text-amber-300"> ({item.dueDate})</span> : null}
                  </li>
                ))}
              </ul>
            </Field>
          ...
          {hasBudget && (
            <Field title="Budget & Cost Breakdown">
              ...
              {useVirtualBudget ? (
                <div className="rounded border border-brand-border/60 overflow-hidden">
                  <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_2fr] text-left text-brand-muted bg-[var(--color-bg-panel)] text-sm">
                    ...
                    <div className="py-2 px-3 font-semibold">Rate</div>
                    <div className="py-2 px-3 font-semibold">Factor</div>
                    <div className="py-2 px-3 font-semibold text-right">Total</div>
                    <div className="py-2 px-3 font-semibold">Rate basis / reason</div>
                  </div>
                  <VirtualizedList
                  ...
                  <table className="min-w-full text-sm text-white border border-brand-border/60 rounded-lg overflow-hidden">
                    <thead>
                      <tr className="text-left text-brand-muted bg-[var(--color-bg-panel)]">
                        ...
                        <th className="py-2 px-3 font-semibold">Rate basis / reason</th>
                      </tr>
                    </thead>
                    <tbody>
              ...
              <div className="mt-2 text-xs text-brand-muted">
                {Number.isFinite(budgetTotal) && budgetTotal > 0 ? `Total: ${formatCurrency(budgetTotal)} from budget line items` : 'Total: —'}
              </div>
            </Field>
          )}
