...
const App: React.FC = () => {
  ...
  const [aiModel] = useState<string>('gpt-5.5');
  ...
  const parseDocument = pidLogic.parseDocument as
    ((text: string, model?: string, initialWarnings?: string[]) => Promise<{ ok: boolean; error?: string }>) |
    undefined;
  ...
  const isAiConfigError = !!errorText && /missing GOOGLE_API_KEY|OPENAI_API_KEY|AI parsing is not configured/i.test(errorText);
  ...
  return (
    <div className="min-h-screen w-full flex flex-col bg-brand-dark text-brand-text" style={shellStyle}>
      {/*
      DEV: debug badge for isCreateMode / centerPid state (disabled).
      {import.meta.env.DEV && (
