...
// Optional model SDKs/providers.
// OpenAI Responses API is the primary path for GPT-5.5.
// Gemini remains as a fallback path if OpenAI credentials are not configured yet.
...
const AUDIT_TRACE_LEVELS = new Set(['meta', 'summary', 'full']);

function getAuditTraceLevel() {
  const level = String(process.env.AUDIT_TRACE_LEVEL || 'summary').trim().toLowerCase();
  return AUDIT_TRACE_LEVELS.has(level) ? level : 'summary';
}

function getAuditMaxFieldChars() {
  const n = Number(process.env.AUDIT_MAX_FIELD_CHARS || 8000);
  return Number.isFinite(n) && n > 0 ? Math.floor(n) : 8000;
}

function getAuditRedactKeys() {
  const raw = process.env.AUDIT_REDACT_KEYS || 'apiKey,password,token,authorization,secret,cookie,bearer';
  return String(raw)
    .split(',')
...
function auditKeyIsSensitive(key) {
  const k = String(key || '').toLowerCase();
  return getAuditRedactKeys().some((needle) => k.includes(needle));
}
...
function auditRedact(value, depth = 0) {
  ...
  if (typeof value === 'string') {
    const bearer = value.replace(/Bearer\s+[A-Za-z0-9._~+/-]+=*/gi, 'Bearer [REDACTED]');
    return bearer.replace(/(api[_-]?key|password|token|secret|authorization|cookie)\s*[:=]\s*[^,\s"}]+/gi, '$1=[REDACTED]');
  }
  if (typeof value !== 'object') return value;
  if (Array.isArray(value)) return value.slice(0, 50).map((v) => auditRedact(v, depth + 1));
  const out = {};
  for (const [k, v] of Object.entries(value)) {
    out[k] = auditKeyIsSensitive(k) ? '[REDACTED]' : auditRedact(v, depth + 1);
  }
  return out;
}

function auditShape(value) {
  if (value === null || value === undefined) return value;
  if (typeof value === 'string') return { type: 'string', lengthChars: value.length };
...
function auditField(value) {
  const level = getAuditTraceLevel();
  if (level === 'meta') return auditShape(value);
  const redacted = auditRedact(value);
  if (level === 'full') return redacted;
  try {
    const s = typeof redacted === 'string' ? redacted : JSON.stringify(redacted);
    const max = getAuditMaxFieldChars();
    return {
      summary: s.length > max ? `${s.slice(0, max)}... [truncated]` : s,
      lengthChars: s.length,
      truncated: s.length > max,
    };
  } catch {
    return auditShape(redacted);
  }
}

function countPopulatedFields(obj) {
  ...
  for (const v of Object.values(obj)) {
    ...
    else if (typeof v === 'object' && Object.values(v).some((x) => x !== null && x !== undefined && String(x).trim() !== '')) count++;
    else if (typeof v === 'number' || typeof v === 'boolean') count++;
  }
  return count;
}
...
function addAuditStep(label, type, input, output, extra = {}) {
  const audit = auditStore.getStore();
  if (!audit) return null;
  const now = new Date().toISOString();
  const step = {
    name: label,
    type: type === 'llm' ? 'llm' : 'deterministic',
    timestamp: now,
    input: auditField(input),
    output: auditField(output),
    ...extra,
  };
  audit.steps.push(step);
  if (extra.modelId && !audit.modelId) audit.modelId = extra.modelId;
  return step;
}

function inferAuditSource(steps) {
  const hasLlm = steps.some((s) => s.type === 'llm');
  const hasDet = steps.some((s) => s.type === 'deterministic');
  if (hasLlm && hasDet) return 'hybrid';
  if (hasLlm) return 'ai';
  return 'deterministic';
}

function buildAuditPayload(audit, payload) {
  const durationMs = Math.max(1, Date.now() - audit.startedAtMs);
  const responsePid = payload?.pid || payload?.pidData || null;
  const warnings = Array.isArray(payload?.warnings) ? payload.warnings : [];
  if (!audit.steps.length) {
    addAuditStep('endpoint_response', 'deterministic', { endpoint: audit.endpoint }, {
      ok: payload?.ok !== false,
      fieldsPopulated: countPopulatedFields(responsePid || payload),
      warningCount: warnings.length,
    });
  }
  const stepLabels = audit.steps.map((s) => s.name);
  const source = inferAuditSource(audit.steps);
  const meta = {
    requestId: audit.requestId,
    endpoint: audit.endpoint,
    modelId: audit.modelId || null,
    source,
    inputLengthChars: audit.inputLengthChars || 0,
    fieldsPopulated: countPopulatedFields(responsePid || payload),
    warningCount: warnings.length,
    durationMs,
...
function attachAuditPayload(audit, payload) {
  if (!payload || typeof payload !== 'object') return payload;
  const { meta, trace } = buildAuditPayload(audit, payload);
  const next = { ...payload, _auditMeta: meta, _auditTrace: trace };
  if (next.pid && typeof next.pid === 'object') next.pid = { ...next.pid, _auditMeta: meta, _auditTrace: trace };
  if (next.pidData && typeof next.pidData === 'object') next.pidData = { ...next.pidData, _auditMeta: meta, _auditTrace: trace };
  auditAiDecision(meta, trace);
  persistAuditTrace(meta, trace);
  return next;
}

function auditAiDecision(meta, trace) {
  emitAuditDecision(meta, trace);
}

function emitAuditDecision(meta, trace) {
  auditLogger.info('ai_decision_trace', {
    ...
    stepLabels: trace.stepLabels,
  });
}

async function persistAuditTrace(meta, trace) {
  const record = { _auditMeta: meta, _auditTrace: trace };
  const writes = [];
  if (process.env.AUDIT_GCS_BUCKET) writes.push(writeAuditToGcs(record));
  if (process.env.AUDIT_BIGQUERY_TABLE) writes.push(writeAuditToBigQuery(record));
  if (process.env.AUDIT_FIRESTORE_COLLECTION) writes.push(writeAuditToFirestore(record));
  if (!writes.length) return;
  Promise.allSettled(writes).then((results) => {
    for (const r of results) {
      if (r.status === 'rejected') console.warn('[audit] optional backend write failed:', r.reason?.message || r.reason);
    }
  });
}

async function writeAuditToGcs(record) {
  const { Storage } = await import('@google-cloud/storage');
  const storage = new Storage();
  const bucket = storage.bucket(process.env.AUDIT_GCS_BUCKET);
  const file = bucket.file(`pmomax-audit/${record._auditMeta.requestId}.json`);
  await file.save(JSON.stringify(record), { contentType: 'application/json' });
}

async function writeAuditToBigQuery(record) {
  const { BigQuery } = await import('@google-cloud/bigquery');
  const [datasetId, tableId] = String(process.env.AUDIT_BIGQUERY_TABLE || '').split('.');
  if (!datasetId || !tableId) throw new Error('AUDIT_BIGQUERY_TABLE must be dataset.table');
  const bq = new BigQuery();
  await bq.dataset(datasetId).table(tableId).insert([{ requestId: record._auditMeta.requestId, record }]);
}

async function writeAuditToFirestore(record) {
  const { Firestore } = await import('@google-cloud/firestore');
  const db = new Firestore();
  await db.collection(process.env.AUDIT_FIRESTORE_COLLECTION).doc(record._auditMeta.requestId).set(record);
}

async function queryAuditBackends(requestId) {
  if (!process.env.AUDIT_GCS_BUCKET && !process.env.AUDIT_BIGQUERY_TABLE && !process.env.AUDIT_FIRESTORE_COLLECTION) {
    return { configured: false, found: false, record: null };
  }
  if (process.env.AUDIT_GCS_BUCKET) {
    try {
      const { Storage } = await import('@google-cloud/storage');
      const storage = new Storage();
      const [buf] = await storage.bucket(process.env.AUDIT_GCS_BUCKET).file(`pmomax-audit/${requestId}.json`).download();
      return { configured: true, found: true, backend: 'gcs', record: JSON.parse(buf.toString('utf8')) };
    } catch (err) {
      if (err?.code && err.code !== 404) console.warn('[audit] gcs lookup failed:', err.message || err);
    }
  }
  if (process.env.AUDIT_FIRESTORE_COLLECTION) {
    try {
      const { Firestore } = await import('@google-cloud/firestore');
      const snap = await new Firestore().collection(process.env.AUDIT_FIRESTORE_COLLECTION).doc(requestId).get();
      if (snap.exists) return { configured: true, found: true, backend: 'firestore', record: snap.data() };
    } catch (err) {
      console.warn('[audit] firestore lookup failed:', err.message || err);
    }
  }
  if (process.env.AUDIT_BIGQUERY_TABLE) {
    try {
      const { BigQuery } = await import('@google-cloud/bigquery');
      const [datasetId, tableId] = String(process.env.AUDIT_BIGQUERY_TABLE || '').split('.');
      const [rows] = await new BigQuery().query({
        query: `SELECT record FROM \`${datasetId}.${tableId}\` WHERE requestId = @requestId LIMIT 1`,
        params: { requestId },
      });
      if (rows[0]?.record) return { configured: true, found: true, backend: 'bigquery', record: rows[0].record };
    } catch (err) {
      console.warn('[audit] bigquery lookup failed:', err.message || err);
    }
  }
  return { configured: true, found: false, record: null };
}

function cloudLoggingInstructions(requestId) {
  const filter = `jsonPayload.message="ai_decision_trace" AND jsonPayload.requestId="${requestId}"`;
  return {
    query: filter,
    gcloud: `gcloud logging read '${filter}' --project "$GOOGLE_CLOUD_PROJECT" --limit 20 --format json`,
  };
}

app.use('/api/ai', (req, res, next) => {
  const audit = createAuditContext(req);
  audit.inputLengthChars = (() => {
  ...
  res.json = (body) => {
    if (!attached) {
      attached = true;
      body = attachAuditPayload(audit, body);
    }
  ...
  res.send = (body) => {
    if (!attached) {
      ...
      if (parsed && typeof parsed === 'object') {
        attached = true;
        const nextBody = attachAuditPayload(audit, parsed);
        res.set('Content-Type', 'application/json; charset=utf-8');
        return originalSend(JSON.stringify(nextBody));
      }
    }
    return originalSend(body);
  };

  return auditStore.run(audit, next);
});
...
async function geminiJson(modelName, systemPrompt, userText) {
  if (process.env.OPENAI_API_KEY) {
    return openaiJson(modelName, systemPrompt, userText);
  }
  ...
  for (const name of modelsToTry) {
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        const output = String(result?.response?.text?.() || '');
        addAuditStep('gemini_json', 'llm', { model: name, systemPrompt, userText }, { output }, {
          modelId: name,
...
async function geminiText(modelName, systemPrompt, userText) {
  if (!GoogleGenerativeAI) throw new Error('Gemini SDK not available.');
  const key = process.env.GOOGLE_API_KEY;
  if (!key) throw new Error('GOOGLE_API_KEY not set.');
  const genAI = new GoogleGenerativeAI(key);
  ...
  for (const name of modelsToTry) {
    try {
      ...
      const output = String(result?.response?.text?.() || '');
      addAuditStep('gemini_text', 'llm', { model: name, systemPrompt, userText }, { output }, {
        modelId: name,
        durationMs: Math.max(1, Date.now() - started),
      });
      return output;
  ...
  throw lastErr || new Error('Gemini text call failed.');
}
...
function openAIModelAliases() {
  return {
    ...
    'gemini-pro-2.5': process.env.OPENAI_MODEL || 'gpt-5.5',
    'gemini-2.5-pro': process.env.OPENAI_MODEL || 'gpt-5.5',
    'gemini-2.5-flash': process.env.OPENAI_FALLBACK_MODEL || 'gpt-5.4-mini',
    'gemini-1.5-flash': process.env.OPENAI_FALLBACK_MODEL || 'gpt-5.4-mini',
  };
}

function resolveOpenAIModel(requestedModel, purpose = 'default') {
  const aliases = openAIModelAliases();
  const requested = String(requestedModel || '').trim();
  if (requested && aliases[requested]) return aliases[requested];
  if (requested && /^gpt-/.test(requested)) return requested;

  const envCandidates =
    purpose === 'qa'
      ? [process.env.OPENAI_ASSISTANT_QA_MODEL, process.env.OPENAI_ASSISTANT_MODEL, process.env.OPENAI_MODEL, process.env.OPENAI_FALLBACK_MODEL]
      : purpose === 'budget'
        ? [process.env.OPENAI_BUDGET_MODEL, process.env.OPENAI_MODEL, process.env.OPENAI_FALLBACK_MODEL]
        : purpose === 'parse'
          ? [process.env.OPENAI_PARSE_MODEL, process.env.OPENAI_MODEL, process.env.OPENAI_FALLBACK_MODEL]
          : [process.env.OPENAI_ASSISTANT_MODEL, process.env.OPENAI_MODEL, process.env.OPENAI_FALLBACK_MODEL];

  const picked = envCandidates.find((v) => typeof v === 'string' && v.trim());
  return picked || (purpose === 'qa' ? 'gpt-5.4-mini' : 'gpt-5.5');
}
...
async function openaiResponsesRequest({ model, instructions, input, json = false, reasoningEffort = 'medium', maxOutputTokens = 8000 }) {
  ...
  const body = {
    ...
    text: { format: { type: json ? 'json_object' : 'text' } },
  };
  ...
  if (!res.ok) {
    const apiMsg = payload?.error?.message || payload?.message || `OpenAI Responses API failed (${res.status})`;
    throw new Error(String(apiMsg));
  }
  const output = extractOpenAIOutputText(payload);
  addAuditStep(json ? 'openai_json' : 'openai_text', 'llm', { model, instructions, input, json, reasoningEffort }, { output }, {
    modelId: model,
    durationMs: Math.max(1, Date.now() - started),
  });
  return output;
...
async function openaiJson(modelName, systemPrompt, userText, purpose = 'default') {
  const model = resolveOpenAIModel(modelName, purpose);
  return openaiResponsesRequest({
    model,
    instructions: systemPrompt,
    input: userText,
    json: true,
    reasoningEffort: purpose === 'qa' ? 'low' : 'medium',
  });
}

async function openaiText(modelName, systemPrompt, userText, purpose = 'default') {
  const model = resolveOpenAIModel(modelName, purpose);
  return openaiResponsesRequest({
    model,
    instructions: systemPrompt,
...
app.get('/api/audit/:requestId', async (req, res) => {
  ...
  if (!requestId) return sendJson(res, { ok: false, error: 'Missing requestId.' }, 400);
  const instructions = cloudLoggingInstructions(requestId);
  try {
    const result = await queryAuditBackends(requestId);
    if (!result.configured) {
      return sendJson(res, {
        ok: true,
        requestId,
        queryable: false,
        reason: 'No optional audit backend configured. Audit traces are emitted to Cloud Logging via stdout.',
        cloudLogging: instructions,
      });
    }
    if (result.found) {
      return sendJson(res, {
    ...
    return sendJson(res, {
      ...
      queryable: true,
      found: false,
      reason: 'Configured audit backend did not return a record. This is not proof that Cloud Logging lacks the trace.',
      cloudLogging: instructions,
    });
  } catch (err) {
    return sendJson(res, {
      ...
      requestId,
      queryable: false,
      reason: `Audit backend lookup failed: ${safeErrorMessage(err)}`,
      cloudLogging: instructions,
    });
  }
});
...
app.post('/api/ai/risk', async (req, res) => {
  try {
    ...
    addAuditStep('risk_heuristic_analysis', 'deterministic', { pidData, appState }, { reply, risks: suggestions });
    return res.json({ ok: true, reply, risks: suggestions });
  } catch (e) {
    addAuditStep('risk_error', 'deterministic', { body: req.body }, { error: safeErrorMessage(e) });
...
app.post('/api/ai/compliance', async (req, res) => {
  try {
    ...
    addAuditStep('compliance_heuristic_scan', 'deterministic', { pidData, appState }, { reply, checklist });
    return res.json({ ok: true, reply, checklist });
  } catch (e) {
    addAuditStep('compliance_error', 'deterministic', { body: req.body }, { error: safeErrorMessage(e) });
...
app.post('/api/ai/parse', async (req, res) => {
  try {
    ...
    const modelName = process.env.OPENAI_PARSE_MODEL || process.env.OPENAI_MODEL || process.env.GEMINI_MODEL || process.env.GEMINI_FALLBACK_MODEL || 'gpt-5.5';
    ...
    const fallbackModelName = process.env.OPENAI_FALLBACK_MODEL || process.env.GEMINI_FALLBACK_MODEL || 'gpt-5.4-mini';
    const modelTryOrder = [modelName, fallbackModelName].filter((m, i, a) => m && a.indexOf(m) === i);

    for (const m of modelTryOrder) {
      try {
        aiRaw = isOpenAIConfigured()
          ? await openaiJson(m, PARSE_SYSTEM_PROMPT, parseTextWrapped, 'parse')
          : await geminiJson(m, PARSE_SYSTEM_PROMPT, parseTextWrapped);
    ...
    addAuditStep('parse_deterministic_extraction', 'deterministic', { fileName, text: parseText }, { pid: detPid, localWarnings });
...
app.post('/api/ai/budget', async (req, res) => {
  try {
    ...
    addAuditStep('budget_deterministic_baseline', 'deterministic', { pidData, contextText, appState }, { pid: baseResponsePid });
    ...
    if (!isOpenAIConfigured() && (!GoogleGenerativeAI || !process.env.GOOGLE_API_KEY)) {
      return sendJson(res, { ok: true, pid: baseResponsePid, warnings: ['AI budget not configured; deterministic baseline applied.'] });
    }

    const modelName = process.env.OPENAI_BUDGET_MODEL || process.env.OPENAI_MODEL || process.env.GEMINI_BUDGET_MODEL || process.env.GEMINI_MODEL || process.env.GEMINI_FALLBACK_MODEL || 'gpt-5.5';
    ...
    let raw = '';
    try {
      raw = isOpenAIConfigured()
        ? await openaiJson(modelName, BUDGET_SYSTEM_PROMPT, budgetInput, 'budget')
        : await geminiJson(modelName, BUDGET_SYSTEM_PROMPT, budgetInput);
    } catch (err) {
    ...
    addAuditStep('budget_ai_merge', 'deterministic', { baseItems, aiItems }, { pid: next });
...
app.post('/api/ai/assistant', async (req, res) => {
  try {
    ...
    const cleanProjectRequest = (requestText) => {
      let text = String(requestText || '').trim();
      text = text.replace(/^["'“”]+|["'“”]+$/g, '').trim();
      text = text.replace(/^(please\s+)?(create|draft|generate|build|write|make)\s+(a|an)?\s*/i, '');
      text = text.replace(/^(pid|project initiation document|project plan)\s+(for|about|to)\s+/i, '');
      text = text.replace(/^for\s+/i, '');
      return text.trim().replace(/\s+/g, ' ');
    };

    const inferDurationDays = (text) => {
      const t = String(text || '').toLowerCase();
      const duration = t.match(/\b(\d+|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)[-\s]*(day|week|month|quarter)s?\b/i);
      if (duration) {
        const n = parseSmallNumber(duration[1]) || 1;
        const unit = duration[2].toLowerCase();
        if (unit === 'day') return Math.max(14, n);
        if (unit === 'week') return Math.max(21, n * 7);
        if (unit === 'quarter') return n * 90;
        return n * 30;
      }
      if (/\bquarterly\b/i.test(t)) return 90;
      return 60;
    };

    const inferTeamSize = (text) => {
      const m = String(text || '').match(/\bteam of (\d+|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)\b/i);
      const n = m ? parseSmallNumber(m[1]) : null;
      return n && n > 0 ? n : null;
    };

    const inferProjectTitle = (requestText) => {
      const clean = cleanProjectRequest(requestText);
      if (/\b(purple[-\s]?team|ttp|adversary|fin6|lapsus|apt41|detection logic|incident response)\b/i.test(clean)) {
        return 'Continuous Purple Teaming Operationalization';
      }
      if (/\b(customer success|support ticket|onboarding|proactive outreach|saas)\b/i.test(clean)) {
        return 'AI Customer Success Automation';
      }
      if (/\b(data platform|migration|warehouse|lakehouse)\b/i.test(clean)) {
        return 'Data Platform Migration';
      }
      const withoutDuration = clean
        .replace(/\b(\d+|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)[-\s]*(day|week|month|quarter)s?\b/gi, '')
        .replace(/\bproject\b/gi, '')
        .replace(/\bteam of \w+\b/gi, '')
        .replace(/[.;:].*$/g, '')
        .trim();
      const words = withoutDuration.split(/\s+/).filter(Boolean).slice(0, 8);
      if (words.length >= 3) {
        return words
          .join(' ')
          .replace(/\b\w/g, (c) => c.toUpperCase())
          .replace(/\b(A|An|The|And|Or|For|Of|To|Into|With)\b/g, (w) => w.toLowerCase())
          .replace(/^\w/, (c) => c.toUpperCase());
      }
      return 'New Project Initiative';
    };

    const summarizeProjectRequest = (requestText, title) => {
      const clean = cleanProjectRequest(requestText);
      const teamSize = inferTeamSize(clean);
      const durationDays = inferDurationDays(clean);
      const durationLabel = durationDays <= 21 ? `${durationDays}-day` : durationDays % 30 === 0 ? `${durationDays / 30}-month` : `${Math.round(durationDays / 7)}-week`;

      if (/\b(purple[-\s]?team|ttp|adversary|fin6|lapsus|apt41|detection logic|incident response)\b/i.test(clean)) {
        const actors = ['FIN6', 'Lapsus$', 'APT41'].filter((a) => clean.toLowerCase().includes(a.toLowerCase()));
        const actorText = actors.length ? `, with explicit coverage for ${actors.join(', ')}` : '';
        return `Operationalize adversary TTP simulation findings into a continuous, automated purple-teaming program${teamSize ? ` for a ${teamSize}-person cybersecurity team` : ''}. The ${durationLabel} project will run after each quarterly simulation, identify detection and response gaps, and enforce detection-rule and incident-response playbook updates within 48 hours${actorText}.`;
      }

      if (/\b(customer success|support ticket|onboarding|proactive outreach|saas)\b/i.test(clean)) {
        return 'Build an integrated AI agent to streamline onboarding, auto-triage support tickets, and perform proactive outreach to reduce manual effort and improve customer satisfaction.';
      }
      ...
      const concise = firstClause.length > 220 ? `${firstClause.slice(0, 217).replace(/\s+\S*$/, '')}...` : firstClause;
      return `${title} will deliver ${concise.charAt(0).toLowerCase()}${concise.slice(1)}. The project will define clear scope, measurable outcomes, accountable owners, delivery milestones, and operational controls for a successful launch.`;
    };
    ...
    const buildCanonicalPid = (requestText) => {
      ...
      const cleanRequest = cleanProjectRequest(raw);
      const title = inferProjectTitle(raw);
      const durationDays = inferDurationDays(raw);
      const teamSize = inferTeamSize(raw);
      const isCyberPurple = /\b(purple[-\s]?team|ttp|adversary|fin6|lapsus|apt41|detection logic|incident response)\b/i.test(cleanRequest);
      const pidId = `PID-${today.replace(/-/g, '')}-${Math.floor(now.getTime() / 1000)}`;

      if (isCyberPurple) {
        const buildEnd = Math.max(21, Math.floor(durationDays * 0.65));
        return mergeWithEmpty({
          ...
          executiveSummary: summarizeProjectRequest(raw, title),
          problemStatement:
            'Quarterly adversary simulations produce valuable findings, but detection logic, response playbooks, and ownership actions can remain manual, inconsistent, and slow to operationalize. This leaves repeatable gaps against financially motivated and state-sponsored threat techniques.',
          businessCaseExpectedValue:
            'The project hardens the defensive posture by converting simulation evidence into repeatable detection engineering, response validation, and governance actions. Expected value includes faster control updates, reduced dwell-time risk, stronger auditability, and measurable closure of TTP-driven detection and response gaps.',
          objectivesSmart: [
            { objective: 'Operationalize simulation findings within 48 hours', successMeasure: '100% of critical findings converted into detection/playbook actions within 48 hours of each quarterly simulation' },
            { objective: 'Automate gap identification and routing', successMeasure: 'Automated gap report generated after each simulation with owner, severity, due date, and evidence link' },
            { objective: 'Validate coverage for FIN6, Lapsus$, and APT41 techniques', successMeasure: 'Mapped detection and response coverage for prioritized ATT&CK techniques used by all three actor profiles' },
            { objective: 'Establish continuous purple-team governance', successMeasure: 'Weekly closure review and monthly control validation dashboard operating by project end' },
          ],
          kpis: [
            { kpi: 'Critical finding update SLA', baseline: 'Manual / inconsistent', target: '≤48 hours' },
            { kpi: 'Detection gap closure rate', baseline: 'TBD from latest simulation', target: '≥90% of high-priority gaps closed by project end' },
            { kpi: 'Playbook update completion', baseline: 'Ad hoc', target: '100% of impacted playbooks updated and reviewed' },
            { kpi: 'Automated report generation time', baseline: 'Manual analysis', target: '≤2 hours after simulation completion' },
          ],
          scopeInclusions: [
            'Simulation findings ingestion and normalization',
            'Adversary technique mapping for FIN6, Lapsus$, and APT41',
            'Automated detection and response gap scoring',
            'Detection logic update workflow with ownership and evidence',
            'Incident response playbook update workflow and approval gates',
            'Purple-team dashboard, weekly review cadence, and 48-hour SLA tracking',
          ],
          scopeExclusions: [
            'Full SIEM replacement or enterprise security platform migration',
            'New red-team simulation design outside quarterly exercises',
            'Remediation of unrelated infrastructure vulnerabilities not tied to simulation findings',
          ],
          assumptions: [
            { assumption: 'Latest adversary simulation outputs are available in a structured or exportable format.' },
            { assumption: 'Security engineering can update detection logic within the target tooling.' },
            { assumption: 'Incident response owners can review and approve playbook changes within the 48-hour SLA.' },
            { assumption: teamSize ? `A ${teamSize}-person delivery team is available for the full project window.` : 'A focused delivery team is available for the full project window.' },
          ],
          constraints: [
            { constraint: `${durationDays}-day delivery window requires strict prioritization of high-impact techniques and controls.` },
            { constraint: 'Production detection changes must follow change-control and false-positive review standards.' },
            { constraint: 'Sensitive adversary simulation details must be handled under security data access controls.' },
          ],
          dependencies: [
            { dependency: 'Quarterly TTP simulation results', teamOrSystem: 'Purple Team / Red Team', status: 'Required' },
            { dependency: 'SIEM / detection engineering access', teamOrSystem: 'Security Engineering', status: 'Required' },
            { dependency: 'Incident response playbook repository', teamOrSystem: 'Security Operations', status: 'Required' },
            { dependency: 'ATT&CK technique mapping and threat intel context', teamOrSystem: 'Threat Intelligence', status: 'Planned' },
          ],
          stakeholders: [
            { name: 'Security Program Sponsor', role: 'Executive Sponsor', contact: 'TBD' },
            { name: 'Purple Team Lead', role: 'Product Owner', contact: 'TBD' },
            { name: 'Detection Engineering Lead', role: 'Technical Owner', contact: 'TBD' },
            { name: 'SOC / Incident Response Lead', role: 'Operational Owner', contact: 'TBD' },
            { name: 'Threat Intelligence Analyst', role: 'Actor/TTP Advisor', contact: 'TBD' },
          ],
          projectSponsor: { name: 'Security Program Sponsor', role: 'Executive Sponsor' },
          projectManagerOwner: { name: 'Purple Team Lead', role: 'Project Owner' },
          teamRaci: [
            { teamMember: 'Purple Team Lead', role: 'Delivery Lead', responsible: 'X', accountable: 'X', consulted: 'X', informed: 'X' },
            { teamMember: 'Detection Engineer', role: 'Detection Logic Updates', responsible: 'X', accountable: '', consulted: 'X', informed: 'X' },
            { teamMember: 'SOC / IR Lead', role: 'Playbook Updates', responsible: 'X', accountable: '', consulted: 'X', informed: 'X' },
            { teamMember: 'Threat Intel Analyst', role: 'FIN6 / Lapsus$ / APT41 Mapping', responsible: 'X', accountable: '', consulted: 'X', informed: '' },
          ],
          timelineOverview:
            `Kickoff and findings intake (${d(0)}-${d(5)}), TTP mapping and gap analysis (${d(5)}-${d(14)}), automation and workflow build (${d(14)}-${d(buildEnd)}), validation and operating handoff (${d(buildEnd)}-${d(durationDays)}).`,
          milestones: [
            { milestone: 'Kickoff and access confirmed', targetDate: d(2) },
            { milestone: 'Simulation findings normalized', targetDate: d(5) },
            { milestone: 'FIN6 / Lapsus$ / APT41 technique map approved', targetDate: d(10) },
            { milestone: 'Detection and response gap report automated', targetDate: d(21) },
            { milestone: '48-hour update workflow live', targetDate: d(buildEnd) },
            { milestone: 'Operating handoff complete', targetDate: d(durationDays) },
          ],
          workBreakdownTasks: [
            { name: 'Kickoff, access, and simulation artifact intake', start: d(0), end: d(3), owner: 'Purple Team Lead', status: 'Planned', priority: 'High', kind: 'Task', dependencies: [] },
            { name: 'Normalize quarterly simulation findings', start: d(3), end: d(7), owner: 'Threat Intel Analyst', status: 'Planned', priority: 'High', kind: 'Task', dependencies: ['Kickoff, access, and simulation artifact intake'] },
            { name: 'Map FIN6, Lapsus$, and APT41 techniques to ATT&CK', start: d(5), end: d(12), owner: 'Threat Intel Analyst', status: 'Planned', priority: 'High', kind: 'Task', dependencies: ['Normalize quarterly simulation findings'] },
            { name: 'Assess detection coverage and false-negative gaps', start: d(10), end: d(18), owner: 'Detection Engineer', status: 'Planned', priority: 'High', kind: 'Task', dependencies: ['Map FIN6, Lapsus$, and APT41 techniques to ATT&CK'] },
            { name: 'Assess response playbook gaps and escalation delays', start: d(10), end: d(18), owner: 'SOC / IR Lead', status: 'Planned', priority: 'High', kind: 'Task', dependencies: ['Map FIN6, Lapsus$, and APT41 techniques to ATT&CK'] },
            { name: 'Build automated gap report and owner assignment workflow', start: d(18), end: d(30), owner: 'Purple Team Lead', status: 'Planned', priority: 'High', kind: 'Task', dependencies: ['Assess detection coverage and false-negative gaps', 'Assess response playbook gaps and escalation delays'] },
            { name: 'Implement detection update SLA tracking', start: d(27), end: d(40), owner: 'Detection Engineer', status: 'Planned', priority: 'High', kind: 'Task', dependencies: ['Build automated gap report and owner assignment workflow'] },
            { name: 'Implement playbook update and approval workflow', start: d(27), end: d(40), owner: 'SOC / IR Lead', status: 'Planned', priority: 'High', kind: 'Task', dependencies: ['Build automated gap report and owner assignment workflow'] },
            { name: 'Validate workflow against last simulation results', start: d(40), end: d(50), owner: 'Purple Team Lead', status: 'Planned', priority: 'High', kind: 'Task', dependencies: ['Implement detection update SLA tracking', 'Implement playbook update and approval workflow'] },
            { name: 'Operational handoff and quarterly runbook sign-off', start: d(50), end: d(durationDays), owner: 'Security Program Sponsor', status: 'Planned', priority: 'Medium', kind: 'Milestone', dependencies: ['Validate workflow against last simulation results'] },
          ],
          criticalPathBoxes: [
            {
              id: 'CP1',
              label: 'Findings Intake & TTP Mapping',
              startDate: d(0),
              endDate: d(14),
              taskIds: [
                'Kickoff, access, and simulation artifact intake',
                'Normalize quarterly simulation findings',
                'Map FIN6, Lapsus$, and APT41 techniques to ATT&CK',
              ],
            },
            {
              id: 'CP2',
              label: 'Gap Automation & Control Updates',
              startDate: d(10),
              endDate: d(40),
              taskIds: [
                'Assess detection coverage and false-negative gaps',
                'Assess response playbook gaps and escalation delays',
                'Build automated gap report and owner assignment workflow',
                'Implement detection update SLA tracking',
                'Implement playbook update and approval workflow',
              ],
            },
            {
              id: 'CP3',
              label: 'Validation & Operating Handoff',
              startDate: d(40),
              endDate: d(durationDays),
              taskIds: [
                'Validate workflow against last simulation results',
                'Operational handoff and quarterly runbook sign-off',
              ],
            },
          ],
          deliverablesOutputs: [
            { name: 'Normalized simulation findings intake template' },
            { name: 'FIN6 / Lapsus$ / APT41 ATT&CK coverage map' },
            { name: 'Automated detection and response gap report' },
            { name: '48-hour detection logic update workflow' },
            { name: 'Incident response playbook update and approval workflow' },
            { name: 'Purple-team operating dashboard and quarterly runbook' },
          ],
          budgetCostBreakdown: [
            { item: `${teamSize || 4}-person security delivery team`, category: 'Labor', cost: '$TBD' },
            { item: 'SIEM / detection engineering tooling', category: 'Security Tooling', cost: '$TBD' },
            { item: 'Automation and dashboard configuration', category: 'Implementation', cost: '$TBD' },
            { item: 'Contingency for tuning and false-positive remediation', category: 'Buffer', cost: '$TBD' },
          ],
          resourcesTools: [
            { resource: 'SIEM / detection engineering platform', purpose: 'Detection rule updates and validation evidence' },
            { resource: 'Incident response playbook repository', purpose: 'Controlled playbook changes and approvals' },
            { resource: 'Threat intel / ATT&CK mapping workspace', purpose: 'Actor technique mapping and coverage scoring' },
            { resource: 'Purple-team dashboard', purpose: 'SLA, gap closure, and governance reporting' },
          ],
          risks: [
            { risk: 'Detection changes increase false positives if rushed', probability: 'Medium', impact: 'High' },
            { risk: 'Playbook owners miss 48-hour review window', probability: 'Medium', impact: 'High' },
            { risk: 'Simulation artifacts lack enough structure for automation', probability: 'Medium', impact: 'Medium' },
            { risk: 'Actor TTP coverage becomes outdated before the next simulation', probability: 'Low', impact: 'Medium' },
          ],
          mitigationsContingencies: [
            { mitigation: 'Add false-positive review and staged rule rollout', contingency: 'Route high-noise detections to monitoring mode before enforcement' },
            { mitigation: 'Pre-assign detection and playbook owners with escalation path', contingency: 'Escalate missed SLA items to sponsor at 36 hours' },
            { mitigation: 'Define a normalized findings template during intake', contingency: 'Use manual triage for unsupported artifacts in the first cycle' },
            { mitigation: 'Refresh actor technique mapping during weekly review', contingency: 'Add emergent high-risk techniques to the backlog immediately' },
          ],
          issuesDecisionsLog: [
            { issue: 'Confirm source format for simulation findings', decision: 'Use normalized finding records with technique, evidence, owner, severity, and SLA fields', owner: 'Purple Team Lead', date: today },
          ],
          communicationPlan: [
            { audience: 'Core security delivery team', cadence: 'Daily during build', channel: 'Standup / work tracker' },
            { audience: 'Security leadership', cadence: 'Weekly', channel: 'Status digest with gap closure metrics' },
            { audience: 'SOC and IR stakeholders', cadence: 'Twice weekly', channel: 'Workflow review and playbook sign-off' },
          ],
          governanceApprovals: [
            { gate: 'TTP coverage map approval', signoffRequirement: 'Purple Team Lead + Threat Intel approval' },
            { gate: 'Detection update workflow go-live', signoffRequirement: 'Detection Engineering + SOC approval' },
            { gate: 'Quarterly operating model handoff', signoffRequirement: 'Security Sponsor approval' },
          ],
          complianceSecurityPrivacy: [
            { requirement: 'Least-privilege access', notes: 'Restrict simulation artifacts, detection logic, and incident playbooks to approved security personnel' },
            { requirement: 'Change control', notes: 'Detection and playbook updates require auditable owner, approval, and rollback details' },
            { requirement: 'Evidence retention', notes: 'Retain simulation findings, gap decisions, and closure evidence according to security audit policy' },
          ],
          openQuestionsNextSteps: [
            { question: 'Which simulation artifact format will be the authoritative input?', nextStep: 'Confirm export format and required fields during kickoff.' },
            { question: 'Which detection platforms and playbook repositories are in scope?', nextStep: 'Inventory tools and owners before workflow build starts.' },
            { question: 'What severity threshold requires mandatory 48-hour updates?', nextStep: 'Define SLA policy and escalation rules with the sponsor.' },
          ],
          notesBackground:
            'This PID is based on a two-month cybersecurity initiative to convert quarterly adversary TTP simulation findings into a repeatable purple-team operating model. The plan prioritizes rapid detection engineering, incident response playbook updates, evidence retention, and governance controls for financial-crime and state-sponsored threat coverage.',
        });
      }
      ...
      return mergeWithEmpty({
        ...
        deliverablesOutputs: [
          { name: 'Approved project charter and success metrics' },
          { name: 'Prioritized requirements and scope baseline' },
          { name: 'Implemented MVP capabilities' },
          { name: 'Testing and validation evidence' },
          { name: 'Launch readiness package and operating handoff' },
        ],
        budgetCostBreakdown: [
        ...
        notesBackground:
          `This PID was drafted from the user's project brief and converted into a structured initiation plan for ${title}. Update the owner names, budget estimates, baselines, and approval gates as project decisions are confirmed.`,
      });
    };
    ...
    // If no AI provider is configured, run deterministic smart behavior instead of a dead demo response.
    if (!isOpenAIConfigured() && (!GoogleGenerativeAI || !process.env.GOOGLE_API_KEY)) {
...
`;
      ...
      const modelName =
        modelOverride ||
        process.env.OPENAI_ASSISTANT_QA_MODEL ||
        process.env.OPENAI_ASSISTANT_MODEL ||
        process.env.OPENAI_MODEL ||
        process.env.GEMINI_ASSISTANT_QA_MODEL ||
        process.env.GEMINI_MODEL ||
        'gpt-5.5';
      const textOut = (isOpenAIConfigured()
        ? await openaiText(modelName, systemPrompt, prompt, 'qa')
        : await geminiText(modelName, systemPrompt, prompt)).trim();
      const questions =
        textOut ||
...
`;
      ...
      let qaParsed = null;
      try {
        qaText = isOpenAIConfigured()
          ? await openaiJson(modelName, '', qaPrompt, 'qa')
          : await geminiJson(modelName, '', qaPrompt);
...
- In CREATE/DRAFT flows, distill the user prompt into a project title and executive summary. Do NOT echo the prompt, quote the prompt, or write summaries like "This PID defines..." followed by the user's request.
- The Executive Summary must read like the demo examples: 1-3 concise sentences stating the project outcome, operating model, target users/threats/systems, and measurable benefit.
- Project titles must be short business titles, not instructions. Example: "Continuous Purple Teaming Operationalization", not "Create a PID for...".
...
`;
    ...
    try {
      replyText = isOpenAIConfigured()
        ? await openaiJson(modelName, '', assistantPrompt, 'default')
        : await geminiJson(modelName, '', assistantPrompt);
    ...
    const repairCreatedPidNarrative = (pid, requestText) => {
      if (!pid || typeof pid !== 'object') return pid;
      const base = buildCanonicalPid(requestText);
      const title = String(pid?.titleBlock?.projectTitle || '').trim();
      const summary = String(pid?.executiveSummary || '').trim();
      const request = String(requestText || '').trim();
      const clean = cleanProjectRequest(requestText);
      const titleLooksEchoed =
        !title ||
        title.length > 85 ||
        /\b(create|draft|generate|build|write|make)\s+(a|an)?\s*(pid|project initiation document|project plan)\b/i.test(title) ||
        (request && title.length > 40 && request.toLowerCase().includes(title.toLowerCase()));
      const summaryLooksEchoed =
        !summary ||
        /\bThis PID defines the scope, schedule, risks, governance, and delivery plan for\s+["“]/i.test(summary) ||
        /\bCreate a PID for\b/i.test(summary) ||
        (clean && clean.length > 120 && summary.includes(clean.slice(0, 120)));

      const next = { ...pid };
      if (titleLooksEchoed) {
        next.titleBlock = { ...(pid.titleBlock || {}), projectTitle: base.titleBlock.projectTitle };
      }
      if (summaryLooksEchoed) {
        next.executiveSummary = base.executiveSummary;
      }
      const notes = String(next?.notesBackground || '').trim();
      const notesLooksEchoed =
        !notes ||
        /\b(create|draft|generate|build|write|make)\s+(a|an)?\s*(pid|project initiation document|project plan)\b/i.test(notes) ||
        (clean && clean.length > 120 && notes.includes(clean.slice(0, 120)));
      if (notesLooksEchoed) {
        next.notesBackground = base.notesBackground;
      }
      if (!Array.isArray(next.deliverablesOutputs) || next.deliverablesOutputs.length === 0) {
        next.deliverablesOutputs = base.deliverablesOutputs || [];
      }
      if (!Array.isArray(next.criticalPathBoxes) || next.criticalPathBoxes.length === 0) {
        next.criticalPathBoxes = base.criticalPathBoxes || next.criticalPathBoxes || [];
      }
      return next;
    };
    ...
    if (parsed && isPlainObject(parsed)) {
      ...
      if (isWrapper) {
        ...
        if (createIntent && cleanedWrapperPid) {
          ...
          cleanedWrapperPid = repairCreatedPidNarrative(cleanedWrapperPid, lastUserText);
        }
        if (createIntent && !wrapperPid) {
          ...
          const finalPid = sanitizePidObject(repairCreatedPidNarrative(withGantt, lastUserText));
      ...
      if (looksLikePid) {
        ...
        if (createIntent) {
          ...
          cleanedPid = repairCreatedPidNarrative(cleanedPid, lastUserText);
        }
