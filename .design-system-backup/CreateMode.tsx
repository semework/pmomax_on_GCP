...
export const CreateMode = (props: CreateModeProps) => {
		...
		return [
			{
				...
				pid: normalizePid({
					...
					budgetCostBreakdown: [
						{ item: 'ML development', category: 'Labor', cost: '$80,000' },
						{ item: 'Integration', category: 'Labor', cost: '$30,000' },
					],
					resourcesTools: [
			...
			},
			{
				...
				pid: normalizePid({
					...
					budgetCostBreakdown: [
						{ item: 'Sensors + gateways', category: 'Equipment', cost: '$45,000' },
						{ item: 'Installation labor', category: 'Labor', cost: '$15,000' },
						{ item: 'Dashboards + alerting', category: 'Software', cost: '$12,000' },
						{ item: 'Training', category: 'Ops', cost: '$8,000' },
					],
					resourcesTools: [
	...
	const callAssistant = async () => {
		...
		try {
			...
			else setLastError(err?.message || 'Assistant failed');
	...
	const handleFileDrop = async (e: React.DragEvent<HTMLDivElement>) => {
		...
		try {
			...
			setLastError(String(err?.message || err || 'Failed to import file'));
		}
	};
	...
	return (
		...
		>
			...
			<div
				className="mt-2 mb-3 rounded-lg border border-brand-border bg-black/10 p-1.5 sm:p-2 relative z-30 sticky top-0 flex flex-col md:flex-row md:flex-nowrap gap-2 items-stretch"
			...
			>
					...
					>
						...
						<div className="mt-2 flex flex-col gap-2 h-full justify-between min-h-0">
							...
							{!stickyCollapsed && (
								<div
									ref={scrollerRef}
									className="flex flex-col gap-1 flex-1 min-h-8 pr-1 overflow-y-auto scroll-smooth"
									style={{ maxHeight: '500px', overscrollBehavior: 'contain', WebkitOverflowScrolling: 'touch' }}
								>
									{[...chat].reverse().map((m, idx) => {
										const isUser = m.role === 'user';
										return (
											<div
												key={`${m.role}-${chat.length - idx}-${idx}`}
				...
				>
					...
					>
						...
						{!stickyCollapsed && (
							<div className="mt-2 flex flex-col gap-2 pb-0 flex-1" style={{ minHeight: 160, overflow: 'auto' }}>
